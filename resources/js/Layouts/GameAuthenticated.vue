...
import { CalendarDaysIcon, UserIcon, FilterIcon, Trophy } from 'lucide-vue-next';
...
onMounted(async () => {
  try {
    ...
    // Listen for the toggle filters event from the main game component
    window.addEventListener('toggleGameFilters', handleToggleGameFilters);
  } catch (error) {
...
onUnmounted(() => {
  // Clean up any event listeners or timers
    window.removeEventListener('toggleGameFilters', handleToggleGameFilters);
});
</script>
        ...
        <div class="flex-1 pt-4">
          <div class="flex items-center">
            <button
              ...
              class="game-menu-width inline-flex items-center justify-center p-2 rounded-md text-white hover:text-white hover:bg-gray-700 focus:outline-none focus:bg-gray-700 focus:text-white transition duration-150 ease-in-out"
            >
          ...
          <transition name="fade">
            <button
              ...
              class="ml-4 inline-flex items-center justify-center p-2 rounded-md text-gray-400 hover:text-white hover:bg-gray-700 focus:outline-none focus:bg-gray-700 focus:text-white transition duration-150 ease-in-out"
            >
            ...
            <div v-if="hasActiveFilters" class="flex items-center space-x-2 mr-4 ml-10 rounded-lg p-2">
              <span class="text-white text-sm font-medium">Active Filters:</span>
              <div class="flex flex-wrap gap-2">
                <div
                  v-for="filter in activeFiltersDisplay"
                  :key="`${filter.type}-${filter.label}`"
                  class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-teal-600/20 text-white border border-teal-600/30"
                >
                  <!-- Calendar Icon -->
                  <svg
